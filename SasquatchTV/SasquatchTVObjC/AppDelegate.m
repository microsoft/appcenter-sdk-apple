...
- (BOOL)application:(UIApplication *)application
  ...
  [MSAppCenter start:@"68065a02-edbb-4fc3-a323-3b8ca2beae80"
...
- (void)crashes {
  ...
  [MSCrashes
      setUserConfirmationHandler:(^(NSArray<MSErrorReport *> *errorReports) {
        ...
        // Add a "No"-Button and call the notifyWithUserConfirmation-callback
        // with MSUserConfirmationDontSend.
        [alertController
            addCancelActionWithTitle:@"Don't Send"
                             handler:^(UIAlertAction *action) {
                               [MSCrashes notifyWithUserConfirmation:
                                              MSUserConfirmationDontSend];
                             }];

        // Add a "Yes"-Button and call the notifyWithUserConfirmation-callback
        // with MSUserConfirmationSend.
        [alertController
            addDefaultActionWithTitle:@"Send"
                              handler:^(UIAlertAction *action) {
                                [MSCrashes notifyWithUserConfirmation:
                                               MSUserConfirmationSend];
                              }];

        // Add a "Always"-Button and call the
        // notifyWithUserConfirmation-callback with MSUserConfirmationAlways.
        [alertController
            addDefaultActionWithTitle:@"Always Send"
                              handler:^(UIAlertAction *action) {
                                [MSCrashes notifyWithUserConfirmation:
...
- (void)setAppCenterCenterDelegate {
  AppCenterViewController *sasquatchController =
      (AppCenterViewController *)[[self window] rootViewController];
...
- (NSArray<MSErrorAttachmentLog *> *)attachmentsWithCrashes:(MSCrashes *)crashes
  ...
  MSErrorAttachmentLog *attachment1 =
      [MSErrorAttachmentLog attachmentWithText:@"Hello world!"
                                      filename:@"hello.txt"];
  MSErrorAttachmentLog *attachment2 = [MSErrorAttachmentLog
      attachmentWithBinary:[@"Fake image"
