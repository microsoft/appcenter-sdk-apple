...
- (void)testLoadTooManyLogs {
  ...
  OCMStub([self.dbConnectionMock loadDataFromDB:[OCMArg any]]).andReturn([self generateSerializedLogsWithCount:expectedLogsCount+1]);

  // When
  BOOL moreLogsAvailable = [self.sut loadLogsForGroupID:kMSTestGroupID limit:expectedLogsCount withCompletion:^(BOOL succeeded, NSArray<MSLog> *_Nonnull logArray, __attribute__((unused)) NSString *_Nonnull batchId) {
...
- (void)testLoadJustEnoughLogs {
  ...
  // If
  NSUInteger expectedLogsCount = 5;
  OCMStub([self.dbConnectionMock loadDataFromDB:[OCMArg any]]).andReturn([self generateSerializedLogsWithCount:expectedLogsCount]);
...
- (void)testLoadNotEnoughLogs {
  ...
  // When
  BOOL moreLogsAvailable = [self.sut loadLogsForGroupID:kMSTestGroupID limit:limit withCompletion:^(BOOL succeeded, NSArray<MSLog> *_Nonnull logArray, __attribute__((unused)) NSString *_Nonnull batchId) {
