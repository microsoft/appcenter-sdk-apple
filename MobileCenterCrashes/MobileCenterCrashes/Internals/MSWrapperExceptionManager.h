 ...
 * 3. When an MSAppleErrorLog must be generated, a corresponding 
 *    MSWrapperException file is searched for, and if found, its
 *    MSException property is added to the MSAppleErrorLog. The file
 *    is not deleted because there is an additional "data" property
 *    that contains information that the wrapper SDK may want.
 * 4. When an MSErrorReport equivalent needs to be generated by a 
 *    wrapper SDK, it is identical to the actual MSErrorReport, but also
 *    includes the additional data that was saved. This data is accessed
 *    by "loadWrapperExceptionWithUUID". Thus, the file on disk can only
 *    be deleted after all crash callbacks with an MSErrorReport parameter
 *    have completed.
 */
