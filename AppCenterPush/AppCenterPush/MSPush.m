...
#if TARGET_OS_OSX
static void *UserNotificationCenterDelegateContext =
    &UserNotificationCenterDelegateContext;
#endif
...
- (instancetype)init {
  if ((self = [super init])) {
    ...
    /*
     * If there is a user notification center delegate already set by a customer
     * before starting Push, assign the delegate to custom user notification
     * center delegate.
     */
    if (center.delegate) {
    ...
    // Set a delegate that will forward notifications to Push as well as a
    // customer's delegate.
...
+ (BOOL)didReceiveRemoteNotification:(NSDictionary *)userInfo {
  return [[MSPush sharedInstance] didReceiveRemoteNotification:userInfo
...
#if TARGET_OS_OSX
  [NSApp registerForRemoteNotificationTypes:(NSRemoteNotificationTypeSound |
        ...
        requestAuthorizationWithOptions:authOptions
                      completionHandler:^(BOOL granted,
                        ...
                        if (granted) {
                          MSLogVerbose(
                              [MSPush logTag],
                              @"Push notifications authorization was granted.");
                        } else {
                          MSLogVerbose(
                              [MSPush logTag],
                              @"Push notifications authorization was denied.");
                        }
                        if (error) {
...
- (void)didRegisterForRemoteNotificationsWithDeviceToken:(NSData *)deviceToken {
  MSLogVerbose(
      [MSPush logTag],
      @"Registering for push notifications has been finished successfully");
...
- (void)didFailToRegisterForRemoteNotificationsWithError:(NSError *)error {
  MSLogWarning(
      [MSPush logTag],
      @"Registering for push notifications has been finished with error: %@",
      error.localizedDescription);
}
...
- (void)applicationDidFinishLaunching:(NSNotification *)notification {
  [self didReceiveUserNotification:
...
- (void)userNotificationCenter:(NSUserNotificationCenter *)center
  ...
  if ([self.originalUserNotificationCenterDelegate
    ...
    [self.originalUserNotificationCenterDelegate
...
- (void)forwardInvocation:(NSInvocation *)anInvocation {
  ...
  // Testing if the selector is defined in NSUserNotificationCenterDelegate or
  // not.
  struct objc_method_description hasMethod =
      protocol_getMethodDescription(@protocol(NSUserNotificationCenterDelegate),
                                    [anInvocation selector], NO, YES);
  if (hasMethod.name != NULL &&
      [self.originalUserNotificationCenterDelegate
          respondsToSelector:[anInvocation selector]]) {
...
- (BOOL)didReceiveRemoteNotification:(NSDictionary *)userInfo
  ...
  if (customData) {
    ...
    if (![[self class] isEnabled]) {
      MSLogVerbose([MSPush logTag], @"Notification received while Push was "
                                    @"enabled but it is disabled now, discard "
                                    @"the notification.");
      return YES;
    }
    ...
    if ([alert isKindOfClass:[NSDictionary class]]) {
      ...
      /*
       * "alert" value type can be either Dictionary or String. Try one more
       * time if it is a String value even though AppCenterPush doesn't support
       * String value for "alert".
       */
    ...
    // Clean values, sometimes we get NSNull from the info dictionary as object
    // type on optional fields.
    title = ([title isKindOfClass:[NSString class]]) ? title : nil;
    message = ([message isKindOfClass:[NSString class]]) ? message : nil;
    MSLogDebug(
        [MSPush logTag],
        @"Notification received.\nTitle: %@\nMessage:%@\nCustom data: %@",
        title, message, [customData description]);

#if TARGET_OS_OSX

    /*
     * Only call the push delegate if the app is in topmost foreground and the
     * notification is a remote notification or it is a user notification.
     * Otherwise, convert a remote notification to a user notification and
     * handle the notification when a user clicks it from notification center.
     */
