...
+ (BOOL)isRecoverableError:(NSInteger)statusCode {
  ...
  // There are some cases when statusCode is 0, e.g., when server is
  // unreachable. If so, the error will contain more details.
...
+ (BOOL)isSSLConnectionError:(NSError *)error {
  ...
  // Check for error domain and if the error.code falls in the range of SSL
  // connection errors (between -2000 and -1200).
  return ([error.domain isEqualToString:NSURLErrorDomain] &&
...
+ (NSString *)hideSecret:(NSString *)secret {
  ...
  // Hide everything if secret is shorter than the max number of displayed
  // characters.
  NSUInteger appSecretHiddenPartLength =
      (secret.length > kMSMaxCharactersDisplayedForAppSecret
           ? secret.length - kMSMaxCharactersDisplayedForAppSecret
           : secret.length);
  NSString *appSecretHiddenPart =
      [@"" stringByPaddingToLength:appSecretHiddenPartLength
                         withString:kMSHidingStringForAppSecret
                    startingAtIndex:0];
  return [secret
      stringByReplacingCharactersInRange:NSMakeRange(0,
                                                     appSecretHiddenPart.length)
