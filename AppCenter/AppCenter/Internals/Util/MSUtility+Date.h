 ...
 *
 * @discussion
 * Utility function that returns NOW as a NSTimeInterval but in ms instead of
 * seconds with sub-ms precision. We're using NSTimeInterval here instead of
 * long long because we might be interested in sub-millisecond precision which
 * we keep with NSTimeInterval as NSTimeInterval is actually NSDouble.
 */
