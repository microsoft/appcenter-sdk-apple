...
#pragma clang diagnostic pop
  ...
  /*
   * Init zlib. windowBits is 31: (15 max compression rate + 16 gzip header and
   * trailer), memLevel is 8: default memory allocation.
   */
  int initError = deflateInit2(&zStreamStruct, Z_DEFAULT_COMPRESSION,
  ...
  if (initError != Z_OK) {
    ...
    MSLogError(MSAppCenter.logTag, @"Compression failed with error: %@",
               errorMsg);
    return nil;
  }
  ...
  NSMutableData *compressedData =
      [NSMutableData dataWithLength:(NSUInteger)([data length] * 1.01 + 12)];
  ...
  do {
    ...
    zStreamStruct.next_out = (unsigned char *)[compressedData mutableBytes] +
                             zStreamStruct.total_out;

    // Set avail_out to total length - the total already output.
    zStreamStruct.avail_out =
        (unsigned int)([compressedData length] - zStreamStruct.total_out);

    // Call deflate, which will update total_out & return Z_STREAM_END if done,
    // Z_OK if more to do, or an error message.
