...
- (NSString *)serializeLogWithPrettyPrinting:(BOOL)prettyPrint {
  ...
  [self.logs enumerateObjectsUsingBlock:^(
                 id<MSLog> _Nonnull obj, __attribute__((unused)) NSUInteger idx,
                 __attribute__((unused)) BOOL *_Nonnull stop) {
    NSMutableDictionary *dict =
        [(id<MSSerializableObject>)obj serializeToDictionary];
    if (dict) {
  ...
  NSData *jsonData = [NSJSONSerialization dataWithJSONObject:logContainer
                                                     options:printOptions
                                                       error:&error];

  if (!jsonData) {
    MSLogError([MSAppCenter logTag],
               @"Couldn't serialize log container to json: %@",
