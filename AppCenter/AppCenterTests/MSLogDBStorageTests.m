...
- (void)testLoadTooManyLogs {
  ...
  NSMutableArray *expectedLogs =
      [[self generateAndSaveLogsWithCount:expectedLogsCount
              + 1                 groupId:kMSTestGroupId] mutableCopy];
  [expectedLogs removeLastObject];

  // When
  BOOL moreLogsAvailable =
      [self.sut loadLogsWithGroupId:kMSTestGroupId limit:expectedLogsCount withCompletion:^(
              NSArray<id <MSLog>> *_Nonnull logArray,
...
- (void)testLoadJustEnoughLogs {
  ...
  NSArray *expectedLogs =
      [self generateAndSaveLogsWithCount:expectedLogsCount groupId:kMSTestGroupId];
...
- (void)testLoadNotEnoughLogs {
  ...
  // When
  BOOL moreLogsAvailable =
      [self.sut loadLogsWithGroupId:kMSTestGroupId limit:limit withCompletion:^(
              NSArray<id <MSLog>> *_Nonnull logArray,
              NSString *_Nonnull batchId) {
...
- (void)testLoadLogsWhilePendingBatchesFromSameGroupId {
  ...
  [self.sut loadLogsWithGroupId:kMSTestGroupId limit:2 withCompletion:^(NSArray<id <MSLog>> *_Nonnull logArray,
                                                                        NSString *_Nonnull batchId) {
...
- (void)testLoadLogsWhilePendingBatchesFromOtherGroupId {
  ...
  [self.sut loadLogsWithGroupId:kMSAnotherTestGroupId limit:2 withCompletion:^(
          NSArray<id <MSLog>> *_Nonnull logArray,
          NSString *_Nonnull batchId) {
...
- (void)testDeleteLogsWithGroupId {
  ...
  // Then
  assertThatInteger([self.sut countEntriesForTable:kMSLogTableName condition:nil],
                    equalToInteger(0));
  ...
  [self.sut loadLogsWithGroupId:kMSTestGroupId limit:2 withCompletion:^(
...
- (void)testDeleteLogsByBatchIdWithOnlyOnePendingBatch {
  ...
  [self.sut.batches removeAllObjects];
  NSArray
      *savedLogs = [self generateAndSaveLogsWithCount:5 groupId:kMSTestGroupId];
  [self.sut loadLogsWithGroupId:kMSTestGroupId limit:2 withCompletion:^(NSArray <MSLog> *_Nonnull logArray,
                                                                        NSString *batchId) {
        batchIdToDelete = batchId;
        NSPredicate *predicate =
            [NSPredicate predicateWithFormat:@"NOT (self IN %@)", logArray];
  ...
  assertThatInteger([self.sut countEntriesForTable:kMSLogTableName condition:condition],
                    equalToInteger(0));
  assertThat(expectedLogs, is(remainingLogs));
  assertThatInteger(self.sut.batches.count, equalToInteger(0));
}
...
- (void)testDeleteLogsByBatchIdWithPendingBatchesFromOtherGroups {
  ...
  [self.sut loadLogsWithGroupId:kMSTestGroupId limit:2 withCompletion:^(NSArray <MSLog> *_Nonnull logArray,
        ...
        NSPredicate *predicate =
            [NSPredicate predicateWithFormat:@"NOT (self IN %@)", logArray];
        expectedLogs =
            [[savedLogs filteredArrayUsingPredicate:predicate] mutableCopy];
...
- (void)testCommonSchemaLogTargetTokenIsSavedAndRestored {
  ...
  // Then
  [self.sut loadLogsWithGroupId:kMSTestGroupId limit:1 withCompletion:^(NSArray <MSLog> *_Nonnull logArray,
...
- (void)testStorageCapacity {
  ...
  [expectedLogs removeObjectsInRange:NSMakeRange(0,
                                                 expectedLogs.count
                                                     - expectedCapacity)];
...
- (void)testMigration {
  ...
  MSDBSchema *schema0 = @{kMSLogTableName: @[
      @{kMSIdColumnName: @[kMSSQLiteTypeInteger, kMSSQLiteConstraintPrimaryKey,
  ...
  MSDBStorage *storage0 =
      [[MSDBStorage alloc] initWithSchema:schema0 version:0 filename:kMSDBFileName];
...
- (void)testSetStorageSizeBelowMaximumLogSizeFails {
  ...
  XCTestExpectation *expectation =
      [self expectationWithDescription:@"Completion handler invoked."];
...
- (NSArray<id <MSLog>> *)generateAndSaveLogsWithCount:(NSUInteger)count groupId:(NSString *)groupId storage:(MSDBStorage *)storage {
  ...
  for (NSUInteger i = 0; i < count; ++i) {
    ...
    NSString *base64Data =
        [logData base64EncodedStringWithOptions:NSDataBase64EncodingEndLineWithLineFeed];
    NSString *addLogQuery =
        [NSString stringWithFormat:@"INSERT INTO \"%@\" (\"%@\", \"%@\") VALUES ('%@', '%@')", kMSLogTableName, kMSGroupIdColumnName, kMSLogColumnName, groupId, base64Data];
  ...
  truelogCount =
      [storage countEntriesForTable:kMSLogTableName condition:[NSString stringWithFormat:@"\"%@\" = '%@'", kMSGroupIdColumnName, groupId]];
...
- (NSArray<id <MSLog>> *)loadLogsWhere:(nullable NSString *)whereCondition {
  ...
  NSMutableString *selectLogQuery =
      [NSMutableString stringWithFormat:@"SELECT * FROM \"%@\"", kMSLogTableName];
