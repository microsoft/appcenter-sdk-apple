...
- (BOOL)handleUpdate:(MSReleaseDetails *)details {
  ...
  if (!self.releaseDetails || ![self.releaseDetails isEqual:details]) {
    ...
    if (strongDelegate && [strongDelegate respondsToSelector:@selector(distribute:onReleaseAvailableWithDetails:)]) {

      dispatch_async(dispatch_get_main_queue(), ^{
        BOOL customized = [strongDelegate distribute:self onReleaseAvailableWithDetails:details];
        MSLogDebug([MSDistribute logTag], @"onReleaseAvailableWithDetails delegate returned %@.", customized ? @"YES" : @"NO");
        if (!customized) {
