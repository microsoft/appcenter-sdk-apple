...
- (void)testAnalyticsDelegateWithoutImplementations {
  ...
  OCMStub([channelMock enqueueItem:OCMOCK_ANY flags:MSFlagsNone]).andDo(^(NSInvocation *invocation) {
    id<MSLog> log = nil;
  ...
  // When
  [[MSAnalytics sharedInstance].channelUnit enqueueItem:eventLog flags:MSFlagsNone];
...
- (void)testTrackEventWithPropertiesNilWhenAnalyticsDisabled {
  ...
  // When
  OCMReject([channelUnitMock enqueueItem:OCMOCK_ANY flags:MSFlagsNone]);
...
- (void)testTrackEventSetsTagWhenTransmissionTargetProvided {
  ...
  [[MSAnalytics sharedInstance] startWithChannelGroup:channelGroupMock
                              ...
                              transmissionTargetToken:nil
                                      fromApplication:YES];
  OCMStub([channelUnitMock enqueueItem:[OCMArg isKindOfClass:[MSEventLog class]] flags:MSFlagsNone]).andDo(^(NSInvocation *invocation) {
    MSEventLog *log;
    [invocation getArgument:&log atIndex:2];
    tag = log.tag;
  });
...
- (void)testTrackEventWithPropertiesNilAndInvalidName {
  ...
  // When
  OCMExpect([channelUnitMock enqueueItem:OCMOCK_ANY flags:MSFlagsNone]);
...
- (void)testStartWithTransmissionTargetAndAppSecretUsesTransmissionTarget {
  ...
  // Then
  OCMVerify([channelUnitMock enqueueItem:log flags:MSFlagsNone]);
