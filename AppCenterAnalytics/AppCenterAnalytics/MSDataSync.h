...
+ (ConflictResolutionPolicy<T> *)firstWriteWinsPolicy:(Document<T> *)localDocument
                        andConflictResolutionDelegate:(id<MSConflictResolutionDelegate>)delgate;
...
// Read
+ (void)read:(NSString *)partition
        andDocumentId:(NSString *)documentID
      andDocumentType:(Class)type
    completionHandler:(void (^)(Document<T> *document, NSError *error))completionHandler;

// List (need optional signature to configure page size)
+ (void)list:(NSString *)partition
      andDocumentType:(Class)type
    completionHandler:(void (^)(Documents<T> *document, NSError *error))completionHandler;

// Create or replace (upsert) a document
+ (void)createOrReplace:(NSString *)partition
          andDocumentId:(NSString *)documentID
            andDocument:(T)document
      completionHandler:(void (^)(Document<T> *document, NSError *error))completionHandler;

+ (void)createOrReplace:(NSString *)partition
                   andDocumentId:(NSString *)documentID
                     andDocument:(T)document
    withConflictResolutionPolicy:(ConflictResolutionPolicy *)policy
               completionHandler:(void (^)(Document<T> *document, NSError *error))completionHandler;

// Force replace a document
+ (void)replace:(NSString *)partition
        andDocumentId:(NSString *)documentID
          andDocument:(T)document
    completionHandler:(void (^)(Document<T> *document, NSError *error))completionHandler;
...
// Delete a document
+ (void)deleteDocument:(NSString *)partition
         andDocumentId:(NSString *)documentID
