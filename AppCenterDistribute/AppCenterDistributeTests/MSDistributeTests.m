...
static NSString *const kMSTestDownloadedDistributionGroupId =
    @"DOWNLOADEDDISTRIBUTIONGROUPID";
...
- (void)setUp {
  ...
  // Make sure we disable the debug-mode checks so we can actually test the
  // logic.
  [MSDistributeTestUtil mockUpdatesAllowedConditions];

  // MSBasicMachOParser may fail on test projects' main bundle. It's mocked to
  // prevent it.
  ...
  OCMStub([self.alertControllerMock alertControllerWithTitle:OCMOCK_ANY
                                                     message:OCMOCK_ANY])
      .andReturn(self.alertControllerMock);

  // Mock DistributeInfoTracker.
  self.distributeInfoTrackerMock =
      OCMClassMock([MSDistributeInfoTracker class]);
...
- (void)testInstallURL {
  ...
  NSArray *bundleArray = @[ @{
    @"CFBundleURLSchemes" :
        @[ [NSString stringWithFormat:@"appcenter-%@", kMSTestAppSecret] ]
  } ];
  OCMStub([self.bundleMock objectForInfoDictionaryKey:@"CFBundleURLTypes"])
      .andReturn(bundleArray);
  OCMStub([self.bundleMock objectForInfoDictionaryKey:@"MSAppName"])
      .andReturn(@"Something");
  id distributeMock = OCMPartialMock(self.sut);
  OCMStub([distributeMock openURLInSafariViewControllerWith:OCMOCK_ANY
                                                  fromClass:OCMOCK_ANY])
      .andDo(nil);
  ...
  NSURL *url =
      [distributeMock buildTokenRequestURLWithAppSecret:kMSTestAppSecret
                                            releaseHash:kMSTestReleaseHash
                                            isTesterApp:false];
  NSURLComponents *components =
      [NSURLComponents componentsWithURL:url resolvingAgainstBaseURL:NO];
  NSMutableDictionary<NSString *, NSString *> *queryStrings =
      [NSMutableDictionary<NSString *, NSString *> new];
  [components.queryItems enumerateObjectsUsingBlock:^(
                             __kindof NSURLQueryItem *_Nonnull queryItem,
  ...
  [self
      waitForExpectationsWithTimeout:1
                             handler:^(NSError *error) {
                               ...
                               assertThatLong(queryStrings.count,
                                              equalToLong(5));
                               assertThatBool(
                                   [components.path
                                       containsString:kMSTestAppSecret],
                                   isTrue());
                               assertThat(queryStrings[kMSURLQueryPlatformKey],
                                          is(kMSURLQueryPlatformValue));
                               assertThat(
                                   queryStrings[kMSURLQueryRedirectIdKey],
                                   is([NSString
                                       stringWithFormat:
                                           kMSDefaultCustomSchemeFormat,
                                           kMSTestAppSecret]));
                               assertThat(queryStrings[kMSURLQueryRequestIdKey],
                                          notNilValue());
                               assertThat(
                                   queryStrings[kMSURLQueryReleaseHashKey],
                                   equalTo(kMSTestReleaseHash));
                               assertThat(
                                   queryStrings
...
- (void)testOpenURLInSafariViewControllerWithUrl {
  ...
  @try {
    [self.sut
        openURLInSafariViewControllerWith:url
                                fromClass:[SFSafariViewControllerMock class]];
...
- (void)testDefaultInstallUrlWorks {
  ...
  NSURL *tokenRequestURL =
      [self.sut buildTokenRequestURLWithAppSecret:kMSTestAppSecret
                                      releaseHash:kMSTestReleaseHash
                                      isTesterApp:false];
  ...
  XCTAssertTrue(
      [tokenRequestURL.absoluteString hasPrefix:kMSDefaultInstallUrl]);
}
...
- (void)testHandleValidUpdate {
  ...
  OCMStub([distributeMock showConfirmationAlert:OCMOCK_ANY])
...
- (void)testHandleUpdateAfterPostpone {
  ...
  OCMStub([distributeMock isNewerVersion:OCMOCK_ANY])
      .andDo(^(NSInvocation *invocation) {
        ...
        BOOL enabled = NO;
        NSValue *returnValue =
            [NSValue valueWithBytes:&enabled objCType:@encode(BOOL)];
        [invocation setReturnValue:&returnValue];
      });
  int actualCounter = 0;
  MSReleaseDetails *details =
      [self generateReleaseDetailsWithVersion:@"1" andShortVersion:@"1.0"];
  details.id = @1;
  details.downloadUrl = [NSURL URLWithString:@"https://contoso.com/valid/url"];
  details.status = @"available";
  details.mandatoryUpdate = false;
  [MS_USER_DEFAULTS
      setObject:[NSNumber numberWithLongLong:(long long)
                                                 [MSUtility nowInMilliseconds] -
                                             100000]
...
- (void)testShowConfirmationAlert {
  ...
  OCMStub([self.bundleMock objectForInfoDictionaryKey:@"CFBundleDisplayName"])
      .andReturn(appName);
  MSReleaseDetails *details = [MSReleaseDetails new];
  ...
  details.releaseNotesUrl =
      [NSURL URLWithString:@"https://contoso.com/release_notes"];
  details.mandatoryUpdate = false;
...
#pragma clang diagnostic pop
  ...
  XCTestExpectation *expection = [self
      expectationWithDescription:@"Confirmation alert has been displayed"];
  ...
  [self waitForExpectationsWithTimeout:1
                               handler:^(NSError *error) {
                                 ...
                                 OCMVerify([self.alertControllerMock
                                     alertControllerWithTitle:OCMOCK_ANY
                                                      message:message]);
                                 OCMVerify([self.alertControllerMock
                                     ...
                                     addDefaultActionWithTitle:
                                         MSDistributeLocalizedString(
                                             @"MSDistributeViewReleaseNotes")
                                                       handler:OCMOCK_ANY]);
                                 OCMVerify([self.alertControllerMock
                                     addPreferredActionWithTitle:OCMOCK_ANY
                                                         handler:OCMOCK_ANY]);
                               }];
}
...
- (void)
    testShowConfirmationAlertWithoutViewReleaseNotesButtonForMandatoryUpdate {
...
- (void)testShowConfirmationAlertForMandatoryUpdateWhileNoNetwork {
  ...
  XCTestExpectation *expection = [self
      expectationWithDescription:
          @"Confirmation alert for private distribution has been displayed"];
...
#pragma clang diagnostic pop
  ...
  OCMStub([reachabilityMock currentReachabilityStatus])
      .andDo(^(NSInvocation *invocation) {
  ...
  expection = [self
      expectationWithDescription:
          @"Confirmation alert for public distribution has been displayed"];

  // When
  [self.sut checkLatestRelease:nil
           distributionGroupId:@"whateverGroupId"
                   releaseHash:@"whateverReleaseHash"];
  dispatch_async(dispatch_get_main_queue(), ^{
    [expection fulfill];
...
- (void)testDontShowConfirmationAlertIfNoMandatoryReleaseWhileNoNetwork {
  ...
  // Mock alert.
  OCMReject([self.alertControllerMock alertControllerWithTitle:OCMOCK_ANY
                                                       message:OCMOCK_ANY]);
  OCMReject([self.alertControllerMock addDefaultActionWithTitle:OCMOCK_ANY
                                                        handler:OCMOCK_ANY]);
  OCMReject([self.alertControllerMock addCancelActionWithTitle:OCMOCK_ANY
                                                       handler:OCMOCK_ANY]);
...
- (void)testCheckLatestReleaseRemoveKeysOnNonRecoverableError {
  ...
  [self waitForExpectationsWithTimeout:1
                               handler:^(NSError *error) {
                                 ...
                                 OCMVerify([self.settingsMock
                                     removeObjectForKey:
                                         kMSUpdateTokenRequestIdKey]);
                                 OCMVerify([self.settingsMock
                                     removeObjectForKey:
                                         kMSPostponedTimestampKey]);
                                 OCMVerify([self.settingsMock
                                     removeObjectForKey:
                                         kMSDistributionGroupIdKey]);
                                 OCMVerify([self.distributeInfoTrackerMock
                                                removeDistributionGroupId]);
                                 XCTAssertNil([self.settingsMock
                                     objectForKey:
                                         kMSSDKHasLaunchedWithDistribute]);
                                 XCTAssertNil([self.settingsMock
                                     objectForKey:kMSUpdateTokenRequestIdKey]);
                                 XCTAssertNil([self.settingsMock
                                     objectForKey:kMSPostponedTimestampKey]);
                                 XCTAssertNil([self.settingsMock
                                     objectForKey:kMSDistributionGroupIdKey]);
                                 if (error) {
                                   XCTFail(@"Expectation Failed with error: %@",
                                           error);
                                 }
...
- (void)testPersistLastestMandatoryUpdate {
  ...
  // Then
  NSMutableDictionary *persistedDict =
      [self.settingsMock objectForKey:kMSMandatoryReleaseKey];
  MSReleaseDetails *persistedRelease =
      [[MSReleaseDetails alloc] initWithDictionary:persistedDict];
...
- (void)testOpenUrlWithInvalidUrl {
  ...
  NSString *scheme = [NSString
      stringWithFormat:kMSDefaultCustomSchemeFormat, kMSTestAppSecret];
  id distributeMock = OCMPartialMock(self.sut);
  OCMReject([distributeMock checkLatestRelease:OCMOCK_ANY
  ...
  // If
  url = [NSURL URLWithString:[NSString stringWithFormat:@"%@://?request_id=%@",
                                                        scheme, requestId]];
...
- (void)testOpenUrlWithCheckLatestRelease {
  ...
  NSString *scheme = [NSString
      stringWithFormat:kMSDefaultCustomSchemeFormat, kMSTestAppSecret];
  id distributeMock = OCMPartialMock(self.sut);
  OCMStub([distributeMock checkLatestRelease:OCMOCK_ANY
                         distributionGroupId:OCMOCK_ANY
                                 releaseHash:kMSTestReleaseHash])
      .andDo(nil);
  OCMStub([distributeMock sharedInstance]).andReturn(distributeMock);
  ...
  NSURL *url = [NSURL
      URLWithString:[NSString
                        stringWithFormat:@"%@://?request_id=%@&update_token=%@",
                                         scheme, requestId, token]];
  ...
  url = [NSURL
      URLWithString:
          [NSString
              stringWithFormat:@"%@://?request_id=%@&distribution_group_id=%@",
                               scheme, requestId, distributionGroupId]];
  ...
  // Then
  assertThatBool(result, isTrue());
  OCMVerify([distributeMock checkLatestRelease:nil
...
- (void)testOpenUrlWithUpdateSetupFailure {
  ...
  NSURL *url = [NSURL
      URLWithString:
          [NSString stringWithFormat:
                        @"%@://?request_id=%@&update_setup_failed=%@", scheme,
                        requestId,
  ...
  assertThatBool(result, isTrue());
  OCMVerify(
      [distributeMock showUpdateSetupFailedAlert:updateSetupFailureMessage]);
...
- (void)testCheckForUpdatesInvalidEnvironment {
  ...
  id utilityMock = OCMClassMock([MSUtility class]);
  OCMStub([appCenterMock isDebuggerAttached]).andReturn(NO);
  OCMStub([utilityMock currentAppEnvironment])
      .andReturn(MSEnvironmentTestFlight);
...
- (void)testSetupUpdatesWithPreviousFailureOnSamePackageHash {
  ...
  // If
  [self.settingsMock setObject:kMSTestReleaseHash
                        forKey:kMSUpdateSetupFailedPackageHashKey];

  // Then
  XCTAssertEqual(
      [self.settingsMock objectForKey:kMSUpdateSetupFailedPackageHashKey],
      kMSTestReleaseHash);
  ...
  // Then
  OCMVerify([distributeMock requestInstallInformationWith:kMSTestReleaseHash]);
  OCMReject([distributeMock buildTokenRequestURLWithAppSecret:OCMOCK_ANY
                                                  releaseHash:kMSTestReleaseHash
                                                  isTesterApp:false]);
  XCTAssertEqual(
      [self.settingsMock objectForKey:kMSUpdateSetupFailedPackageHashKey],
...
- (void)testSetupUpdatesWithPreviousFailureOnDifferentPackageHash {
  ...
  // If
  [self.settingsMock setObject:@"different-release-hash"
                        forKey:kMSUpdateSetupFailedPackageHashKey];
  ...
  // Then
  OCMVerify([distributeMock requestInstallInformationWith:kMSTestReleaseHash]);
  OCMVerify([distributeMock buildTokenRequestURLWithAppSecret:OCMOCK_ANY
                                                  releaseHash:kMSTestReleaseHash
                                                  isTesterApp:false]);
  XCTAssertNil(
      [self.settingsMock objectForKey:kMSUpdateSetupFailedPackageHashKey]);
...
- (void)testBrowserNotOpenedWhenTesterAppUsedForUpdateSetup {
  ...
  id distributeMock = OCMPartialMock(self.sut);
  id utilityMock = [self mockMSPackageHash];
  OCMStub([distributeMock buildTokenRequestURLWithAppSecret:OCMOCK_ANY
                                                releaseHash:OCMOCK_ANY
                                                isTesterApp:false])
  ...
  XCTestExpectation *expectation =
      [self expectationWithDescription:@"Start update processed"];
  ...
  [self waitForExpectationsWithTimeout:1
                               handler:^(NSError *error) {
                                 ...
                                 OCMVerify([distributeMock
                                     openUrlUsingSharedApp:OCMOCK_ANY]);
                                 OCMReject([distributeMock
                                     openUrlInAuthenticationSessionOrSafari:
                                         OCMOCK_ANY]);
                                 if (error) {
                                   XCTFail(@"Expectation Failed with error: %@",
                                           error);
...
- (void)testDeleteUpdateTokenAfterReinstall {
  ...
  OCMVerify([self.settingsMock setObject:@(1)
                                  forKey:kMSSDKHasLaunchedWithDistribute]);
...
- (void)testDismissEmbeddedSafari {
  ...
  // If
  XCTestExpectation *safariDismissedExpectation =
      [self expectationWithDescription:@"Safari dismissed processed"];
  ...
  [self
      waitForExpectationsWithTimeout:1
                             handler:^(NSError *error) {
                               ...
                               assertThat(self.sut.safariHostingViewController,
                                          nilValue());
                               if (error) {
                                 XCTFail(@"Expectation Failed with error: %@",
                                         error);
...
- (void)testDismissEmbeddedSafariWhenOpenURL {
  ...
  OCMStub([distributeMock checkLatestRelease:OCMOCK_ANY
      ...
      .andDo(nil);
  self.sut.appSecret = kMSTestAppSecret;
  [MS_USER_DEFAULTS setObject:@"FIRST-REQUEST"
                       forKey:kMSUpdateTokenRequestIdKey];
  NSDictionary<NSString *, id> *plist =
      @{ @"CFBundleShortVersionString" : @"1.0",
         @"CFBundleVersion" : @"1" };
  OCMStub([self.bundleMock infoDictionary]).andReturn(plist);
  NSURL *url = [NSURL
      URLWithString:
          [NSString stringWithFormat:
                        @"%@://?request_id=FIRST-REQUEST&update_token=token",
...
- (void)testShowDistributeDisabledAlert {
  ...
  XCTestExpectation *expectation =
      [self expectationWithDescription:
                @"Distribute disabled alert has been displayed"];
                             ...
                             handler:^(__attribute__((unused)) NSError *error) {
                               ...
                               OCMVerify([self.alertControllerMock
                                   alertControllerWithTitle:OCMOCK_ANY
                                                    message:nil]);
                               OCMVerify([self.alertControllerMock
                                   addCancelActionWithTitle:OCMOCK_ANY
                                                    handler:OCMOCK_ANY]);
                             }];
}
...
- (void)testStartDownload {
  ...
  id utilityMock = OCMClassMock([MSUtility class]);
  OCMStub(ClassMethod([utilityMock sharedAppOpenUrl:OCMOCK_ANY
                                            options:OCMOCK_ANY
...
- (void)testUpdateURLWithUnregisteredScheme {
  ...
  // If
  NSArray *bundleArray = @[
    @{ @"CFBundleURLSchemes" : @[ @"appcenter-IAMSUPERSECRET" ] }
  ];
  OCMStub([self.bundleMock objectForInfoDictionaryKey:@"CFBundleURLTypes"])
      .andReturn(bundleArray);
...
- (void)testIsNewerVersionFunction {
  ...
  // If
  MSReleaseDetails *newerRelease =
      [self generateReleaseDetailsWithVersion:@"20" andShortVersion:@"20.0"];
  ...
  // If
  MSReleaseDetails *olderRelease =
      [self generateReleaseDetailsWithVersion:@"5" andShortVersion:@"5.0"];
  ...
  // If
  MSReleaseDetails *sameRelease =
      [self generateReleaseDetailsWithVersion:@"10" andShortVersion:@"10.0"];
...
- (void)testStartUpdateWhenApplicationEnterForeground {
  ...
  OCMStub([notificationCenterMock defaultCenter])
      .andReturn(notificationCenterMock);
...
- (void)testNotifyUpdateActionPostpone {
  ...
  [distributeMock setValue:OCMClassMock([MSReleaseDetails class])
                    forKey:@"releaseDetails"];

  // When
  [distributeMock notifyUpdateAction:MSUpdateActionPostpone];

  // Then
  assertThat([self.settingsMock objectForKey:kMSPostponedTimestampKey],
             equalToLongLong((long long)time));
...
- (void)testDefaultUpdateAlert {
  ...
  [self waitForExpectationsWithTimeout:1
                               handler:^(NSError *error) {
                                 ...
                                 OCMVerify([distributeMock
                                     showConfirmationAlert:detailsMock]);
                               }];
...
- (void)testDefaultUpdateAlertWithDelegate {
  ...
  // When
  OCMStub([delegateMock distribute:distributeMock
              releaseAvailableWithDetails:OCMOCK_ANY])
      .andReturn(NO);
  ...
  [self waitForExpectationsWithTimeout:1
                               handler:^(NSError *error) {
                                 ...
                                 OCMVerify([[distributeMock delegate]
...
- (void)testWillNotReportReleaseInstallForPrivateGroupWithoutStoredReleaseHash {
  ...
  // When
  NSMutableDictionary *reportingParametersForUpdatedRelease = [self.sut
...
- (void)
    testWillNotReportReleaseInstallForPrivateGroupWhenReleaseHashesDontMatch {

  // If
  [self.settingsMock setObject:@"ReleaseHash2"
                        forKey:kMSDownloadedReleaseHashKey];
...
- (void)testReportReleaseInstallForPrivateGroupWhenReleaseHashesMatch {
  ...
  // Then
  assertThat(
      reportingParametersForUpdatedRelease[kMSURLQueryDistributionGroupIdKey],
      equalTo(kMSTestDistributionGroupId));
  assertThat(
      reportingParametersForUpdatedRelease[kMSURLQueryDownloadedReleaseIdKey],
      equalTo(@1));
}
...
- (void)testReportReleaseInstallForPublicGroupWhenReleaseHashesMatch {
  ...
  // When
  NSMutableDictionary *reportingParametersForUpdatedRelease = [self.sut
      getReportingParametersForUpdatedRelease:updateToken
                  currentInstalledReleaseHash:kMSTestReleaseHash
                          distributionGroupId:kMSTestDistributionGroupId];

  // Then
  assertThat(reportingParametersForUpdatedRelease[kMSURLQueryInstallIdKey],
             equalTo(installId));
  assertThat(
      reportingParametersForUpdatedRelease[kMSURLQueryDownloadedReleaseIdKey],
      equalTo(@1));
  ...
  [self
      waitForExpectationsWithTimeout:1
                             handler:^(NSError *error) {
                               ...
                               // Then
                               OCMVerify([distributeMock
                                   getReportingParametersForUpdatedRelease:
                                       kMSTestUpdateToken
                                               currentInstalledReleaseHash:
                                                   kMSTestReleaseHash
                                                       distributionGroupId:
                                                           kMSTestDistributionGroupId]);
                               if (error) {
                                 XCTFail(@"Expectation Failed with error: %@",
                                         error);
                               }
...
- (void)testShouldChangeDistributionGroupIdIfStoredIdDoesntMatchDownloadedId {
  ...
  [self.settingsMock setObject:kMSTestReleaseHash
                        forKey:kMSDownloadedReleaseHashKey];
  [self.settingsMock setObject:kMSTestDistributionGroupId
                        forKey:kMSDistributionGroupIdKey];
  [self.settingsMock setObject:kMSTestDownloadedDistributionGroupId
  ...
  // Then
  OCMVerify([distributeMock
      changeDistributionGroupIdAfterAppUpdateIfNeeded:kMSTestReleaseHash]);
  assertThat([self.settingsMock objectForKey:kMSDistributionGroupIdKey],
             equalTo(kMSTestDownloadedDistributionGroupId));
  XCTAssertNil(
      [self.settingsMock objectForKey:kMSDownloadedDistributionGroupIdKey]);
...
- (void)testStoreDownloadedReleaseDetails {
  ...
  // Then
  assertThat([self.settingsMock objectForKey:kMSDownloadedReleaseIdKey],
             equalTo(@1));
  assertThat([self.settingsMock objectForKey:kMSDownloadedReleaseHashKey],
             equalTo(kMSTestReleaseHash));
  assertThat(
      [self.settingsMock objectForKey:kMSDownloadedDistributionGroupIdKey],
      equalTo(kMSTestDistributionGroupId));
...
- (MSReleaseDetails *)generateReleaseDetailsWithVersion:(NSString *)version
                                        andShortVersion:
...
#pragma GCC diagnostic ignored "-Wcast-qual"
  OCMStub(ClassMethod([utilityMock sha256:OCMOCK_ANY]))
      .andReturn(kMSTestReleaseHash);
