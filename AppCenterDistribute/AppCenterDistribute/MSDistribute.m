          ...
          completionHandler:^(__unused NSString *callId, NSUInteger statusCode,
            ...
            if (statusCode == MSHTTPCodesNo200OK) {
              ...
              if (!details) {
                 ...
                 * the URL links generated by the server will be different.
...
#pragma clang diagnostic ignored "-Wobjc-messaging-id"
    ...
    MSCompletionBlockForAuthSession authCompletionBlock = ^(NSURL *callbackUrl,
      ...
      if (error != nil) {
        MSLogDebug([MSDistribute logTag], @"Called %@ with error: %@",
...
- (BOOL)openURL:(NSURL *)url {
  ...
  if ([self isEnabled]) {
    ...
    if (queryDistributionGroupId) {
      ...
      // If Analytics SDK is disabled session Id is null and there is no need
      // to update the distribution session count.
      if (latestSessionId) {
