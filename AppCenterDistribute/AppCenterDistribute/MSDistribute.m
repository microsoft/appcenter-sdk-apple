 ...
 */
static NSString *const kMSTesterAppUpdateTokenPath =
    @"ms-actesterapp://update-setup";

#pragma mark - Error constants

static NSString *const kMSUpdateTokenURLInvalidErrorDescFormat =
    @"Invalid update token URL:%@";
...
- (instancetype)init {
  if ((self = [super init])) {
    ...
    /*
     * Delete update token if an application has been uninstalled and try to get
     * a new one from server. For iOS version < 10.3, keychain data won't be
     * automatically deleted by uninstall so we should detect it and clean up
     * keychain data when Distribute service gets initialized.
     */
    NSNumber *flag =
        [MS_USER_DEFAULTS objectForKey:kMSSDKHasLaunchedWithDistribute];
    if (!flag) {
...
- (MSInitializationPriority)initializationPriority {
  ...
  // Initialize Distribute before Analytics to add distributionGroupId to the
  // first startSession event after app starts.
...
- (void)applyEnabledState:(BOOL)isEnabled {
  ...
  if (isEnabled) {
    ...
    NSString *distributionGroupId =
        [MS_USER_DEFAULTS objectForKey:kMSDistributionGroupIdKey];
    if (distributionGroupId) {
      MSLogDebug([MSDistribute logTag], @"Successfully retrieved distribution "
                                        @"group Id setting it in "
                                        @"distributeInfoTracker.");
      [self.distributeInfoTracker
          updateDistributionGroupId:distributionGroupId];
    }
  } else {
...
- (void)notifyUpdateAction:(MSUpdateAction)action {
  if (!self.releaseDetails) {
    MSLogDebug([MSDistribute logTag],
               @"The release has already been processed.");
    return;
  }
  switch (action) {
  case MSUpdateActionUpdate:
    ...
    if ([self isEnabled]) {
      MSLogDebug(
          [MSDistribute logTag],
          @"'Update now' is selected. Start download and install the update.");

      // Store details to report new download after restart if this release is
      // installed.
      [self storeDownloadedReleaseDetails:self.releaseDetails];
#if TARGET_OS_SIMULATOR

      /*
       * iOS simulator doesn't support "itms-services" scheme, simulator will
       * consider the scheme as an invalid address. Skip download process if the
       * application is running on simulator.
       */
      MSLogWarning([MSDistribute logTag],
                   @"Couldn't download a new release on simulator.");
#else
      [self startDownload:self.releaseDetails];
#endif
    } else {
      MSLogDebug([MSDistribute logTag],
                 @"'Update now' is selected but Distribute was disabled.");
      [self showDistributeDisabledAlert];
    }
    break;
  case MSUpdateActionPostpone:
    MSLogDebug([MSDistribute logTag],
               @"The SDK will ask the update tomorrow again.");
...
- (void)sendFirstSessionUpdateLog {
  ...
  MSDistributionStartSessionLog *log =
      [[MSDistributionStartSessionLog alloc] init];
...
- (void)requestInstallInformationWith:(NSString *)releaseHash {
  ...
  if ([self checkForUpdatesAllowed]) {
    ...
    if ([MS_Reachability reachabilityForInternetConnection]
            .currentReachabilityStatus == NotReachable) {
      MSLogWarning([MSDistribute logTag],
                   @"The device lost its internet connection. The SDK will "
     ...
     * try again. Only if the app build is different (different package hash),
     * try enabling in-app updates again.
     */
    NSString *updateSetupFailedPackageHash =
    ...
    if (updateSetupFailedPackageHash) {
      if ([updateSetupFailedPackageHash isEqualToString:releaseHash]) {
        ...
        MSLogDebug([MSDistribute logTag], @"Re-attempting in-app updates setup "
                                          ...
                                          @"storage.");
        [MS_USER_DEFAULTS
            removeObjectForKey:kMSUpdateSetupFailedPackageHashKey];
        [MS_USER_DEFAULTS removeObjectForKey:kMSTesterAppUpdateSetupFailedKey];
      }
    }
    ...
    MSLogInfo([MSDistribute logTag],
              @"Request information of initial installation.");

    // Don't run on the UI thread, or else the app may be slow to startup.
    NSURL *testerAppUrl = [self buildTokenRequestURLWithAppSecret:self.appSecret
                                                      releaseHash:releaseHash
                                                      isTesterApp:true];
    NSURL *installUrl = [self buildTokenRequestURLWithAppSecret:self.appSecret
                                                    releaseHash:releaseHash
                                                    isTesterApp:false];
    dispatch_async(dispatch_get_main_queue(), ^{
      BOOL shouldUseTesterAppForUpdateSetup =
          [MS_USER_DEFAULTS objectForKey:kMSTesterAppUpdateSetupFailedKey] ==
          NULL;
      BOOL testerAppOpened = NO;
      if (shouldUseTesterAppForUpdateSetup) {
        MSLogInfo([MSDistribute logTag],
                  @"Attempting to use tester app for update setup.");
      ...
      // If the native app could not be opened (not installed), fall back to the
  ...
  if ([self checkForUpdatesAllowed]) {
    ...
    if ([MS_Reachability reachabilityForInternetConnection]
      ...
      MSReleaseDetails *details = [[MSReleaseDetails alloc]
          initWithDictionary:[MS_USER_DEFAULTS
                                 objectForKey:kMSMandatoryReleaseKey]];
    ...
    if (self.ingestion == nil) {
      ...
      if (reportingParametersForUpdatedRelease != nil) {
        [queryStrings
            addEntriesFromDictionary:reportingParametersForUpdatedRelease];
      }
      queryStrings[kMSURLQueryReleaseHashKey] = releaseHash;
      self.ingestion =
          [[MSDistributeIngestion alloc] initWithBaseUrl:self.apiUrl
          ...
          completionHandler:^(__unused NSString *callId, NSUInteger statusCode,
            ...
            if (statusCode == MSHTTPCodesNo200OK) {
              ...
              if (data) {
                ...
                details =
                    [[MSReleaseDetails alloc] initWithDictionary:dictionary];
              }
              if (!details) {
                MSLogError([MSDistribute logTag],
                 ...
                 * Handle this update.
                 *
                 * NOTE: There is one glitch when this release is the same than
                 * the currently displayed mandatory release. In this case the
                 * current UI will be dismissed then redisplayed with the same
                 * UI content. This is an edge case since it's only happening if
                 * there was no network at app start then network came back
                 * along with the same mandatory release from the server. In
                 * addition to that and even though the releases are the same,
                 * the URL links gerenarted by the server will be different.
                 * Thus, there is the overhead of updating the currently
                 * displayed download action with the new URL. In the end fixing
                 * this edge case adds too much complexity for no worthy
                 * advantages, keeping it as it is for now.
                 */
            ...
            // Failure.
              ...
              MSLogDebug([MSDistribute logTag],
                         @"Failed to get an update response, status code: %tu",
                         statusCode);
              ...
              // Check the status code to clean up Distribute data for an
              // unrecoverable error.
              if (![MSIngestionUtil isRecoverableError:statusCode]) {
                ...
                if (dictionary) {
                  details =
                      [[MSErrorDetails alloc] initWithDictionary:dictionary];
                }

                // If the response payload is MSErrorDetails, consider it as a
                // recoverable error.
                if (!details || ![kMSErrorCodeNoReleasesForUser
                                    isEqualToString:details.code]) {
                  [MSKeychainUtil deleteStringForKey:kMSUpdateTokenKey];
                  [MS_USER_DEFAULTS
                      removeObjectForKey:kMSSDKHasLaunchedWithDistribute];
                  [MS_USER_DEFAULTS
                      removeObjectForKey:kMSUpdateTokenRequestIdKey];
                  [MS_USER_DEFAULTS
                      removeObjectForKey:kMSPostponedTimestampKey];
                  [MS_USER_DEFAULTS
                      removeObjectForKey:kMSDistributionGroupIdKey];
    ...
    // Log a message to notify the user why the SDK didn't check for updates.
    MSLogDebug([MSDistribute logTag],
               @"Distribute won't check if a new release is available because "
               @"of one of the following reasons: 1. A debugger "
                "is attached. 2. You are running the debug configuration. 3. "
                "The app is running in a non-adhoc environment. "
                "Detach the debugger and restart the app and/or run the app "
                "with the release configuration to enable the "
                "feature.");
  }
...
- (BOOL)checkURLSchemeRegistered:(NSString *)urlScheme {
  NSArray *schemes;
  NSArray *types =
      [MS_APP_MAIN_BUNDLE objectForInfoDictionaryKey:@"CFBundleURLTypes"];
...
- (nullable NSURL *)buildTokenRequestURLWithAppSecret:(NSString *)appSecret
  ...
  NSString *scheme =
      [NSString stringWithFormat:kMSDefaultCustomSchemeFormat, appSecret];
  if (![self checkURLSchemeRegistered:scheme]) {
    MSLogError([MSDistribute logTag],
               @"Custom URL scheme for Distribute not found.");
    return nil;
  }
  ...
  if (isTesterApp) {
    ...
    NSString *urlPath =
        [NSString stringWithFormat:kMSUpdateTokenApiPathFormat, appSecret];
  ...
  if (!components) {
    MSLogError([MSDistribute logTag], kMSUpdateTokenURLInvalidErrorDescFormat,
               urlString);
    return nil;
  }
  ...
  NSString *requestId =
      [MS_USER_DEFAULTS objectForKey:kMSUpdateTokenRequestIdKey];
  if (!requestId) {
  ...
  [items addObject:[NSURLQueryItem queryItemWithName:kMSURLQueryReleaseHashKey
                                               value:releaseHash]];
  [items addObject:[NSURLQueryItem queryItemWithName:kMSURLQueryRedirectIdKey
                                               value:scheme]];
  [items addObject:[NSURLQueryItem queryItemWithName:kMSURLQueryRequestIdKey
                                               value:requestId]];
  [items addObject:[NSURLQueryItem queryItemWithName:kMSURLQueryPlatformKey
                                               value:kMSURLQueryPlatformValue]];
  [items addObject:[NSURLQueryItem
                       queryItemWithName:
                           kMSURLQueryEnableUpdateSetupFailureRedirectKey
                                   value:@"true"]];
...
/*
 * Only iOS 9.x and 10.x will download the update after users click the
 * "Install" button. We need to force-exit the application for other versions or
 * for any versions when the update is mandatory.
 */
...
- (void)openURLInAuthenticationSessionWith:(NSURL *)url
  ...
  MSLogDebug([MSDistribute logTag],
             @"Using SFAuthenticationSession to open URL: %@", url);
  NSString *callbackUrlScheme =
      [NSString stringWithFormat:kMSDefaultCustomSchemeFormat, self.appSecret];
...
#pragma clang diagnostic ignored "-Wobjc-messaging-id"
    ...
    SEL initSelector = NSSelectorFromString(
        @"initWithURL:callbackURLScheme:completionHandler:");

    // The completion block that we need to invoke.
    typedef void (^MSCompletionBlockForAuthSession)(NSURL *callbackUrl,
                                                    NSError *error);
    MSCompletionBlockForAuthSession authCompletionBlock = ^(NSURL *callbackUrl,
                                                            NSError *error) {
      self.authenticationSession = nil;
      if (error != nil) {
        MSLogDebug([MSDistribute logTag], @"Called %@ with errror: %@",
                   callbackUrl, error.localizedDescription);
      }

      // This is error.code == SFAuthenticationErrorCanceledLogin which we can't
      // use to retain backward compatibility.
      if (error.code == 1) {
        MSLogError([MSDistribute logTag],
                   @"Authentication session was cancelled by user or failed.");
      }
      if (callbackUrl) {
    ...
    typedef void (*MSInitSFAuthenticationSession)(
        id, SEL, NSURL *, NSString *, MSCompletionBlockForAuthSession);
    MSInitSFAuthenticationSession initMethodCall;
    initMethodCall =
        (MSInitSFAuthenticationSession)[session methodForSelector:initSelector];
    initMethodCall(session, initSelector, url, callbackUrlScheme,
                   authCompletionBlock);
    ...
    startMethodCall = (MSStartSFAuthenticationSession)
        [session methodForSelector:startSelector];
    BOOL success = startMethodCall(session, @selector(start));
    if (success) {
      MSLogDebug(
          [MSDistribute logTag],
          @"Authentication Session Started, showing confirmation dialog");
    }
#pragma clang diagnostic pop
  }
}
...
- (void)openURLInSafariViewControllerWith:(NSURL *)url fromClass:(Class)clazz {
  MSLogDebug([MSDistribute logTag],
             @"Using SFSafariViewController to open URL: %@", url);
#pragma clang diagnostic push
...
#pragma clang diagnostic pop
  ...
  [self.safariHostingViewController presentViewController:safari
                                                 animated:YES
                                               completion:nil];
}
...
- (void)dismissEmbeddedSafari {
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    if (strongSelf && strongSelf.safariHostingViewController &&
      ...
      [strongSelf.safariHostingViewController
...
- (BOOL)handleUpdate:(MSReleaseDetails *)details {
  ...
  if (![details.status isEqualToString:@"available"]) {
    MSLogError([MSDistribute logTag],
               @"The new release is not available, skip update.");
    return NO;
  }

  // Step 3. Check if the update is postponed by a user.
  NSNumber *postponedTimestamp =
      [MS_USER_DEFAULTS objectForKey:kMSPostponedTimestampKey];
  if (postponedTimestamp) {
    long long duration = (long long)[MSUtility nowInMilliseconds] -
                         [postponedTimestamp longLongValue];
    if (duration >= 0 && duration < kMSDayInMillisecond) {
      if (details.mandatoryUpdate) {
        MSLogDebug([MSDistribute logTag],
                   @"The update was postponed within a day ago but the update "
                   @"is a mandatory "
                   @"update. The SDK will proceed update for the release.");
      } else {
        MSLogDebug([MSDistribute logTag],
                   @"The update was postponed within a day ago, skip update.");
        return NO;
      }
    }
    [MS_USER_DEFAULTS removeObjectForKey:kMSPostponedTimestampKey];
  ...
  if ([MS_DEVICE.systemVersion compare:details.minOs options:NSNumericSearch] ==
      NSOrderedAscending) {
    MSLogDebug(
        [MSDistribute logTag],
        @"The new release doesn't support this iOS version: %@, skip update.",
  ...
  if (details.mandatoryUpdate) {
    ...
    [MS_USER_DEFAULTS setObject:[details serializeToDictionary]
                         forKey:kMSMandatoryReleaseKey];
  } else {
  ...
  if (!self.releaseDetails || ![self.releaseDetails isEqual:details]) {
    ...
    if (strongDelegate &&
        [strongDelegate respondsToSelector:@selector
                        (distribute:releaseAvailableWithDetails:)]) {
      dispatch_async(dispatch_get_main_queue(), ^{
        BOOL customized = [strongDelegate distribute:self
                         releaseAvailableWithDetails:details];
        MSLogDebug([MSDistribute logTag],
                   @"releaseAvailableWithDetails delegate returned %@.",
                   customized ? @"YES" : @"NO");
        if (!customized) {
...
- (void)storeDownloadedReleaseDetails:(nullable MSReleaseDetails *)details {
  ...
  /*
   * IPA can contain several hashes, each for different architecture and we
   * can't predict which will be installed, so save all hashes as comma
   * separated string.
   */
  NSString *releaseHashes =
      [details.packageHashes count] > 0
          ? [details.packageHashes componentsJoinedByString:@","]
          : nil;
  [MS_USER_DEFAULTS setObject:groupId
                       forKey:kMSDownloadedDistributionGroupIdKey];
...
- (void)removeDownloadedReleaseDetailsIfUpdated:
  ...
  NSString *lastDownloadedReleaseHashes =
      [MS_USER_DEFAULTS objectForKey:kMSDownloadedReleaseHashKey];
  if (lastDownloadedReleaseHashes == nil) {
    return;
  }
  if ([lastDownloadedReleaseHashes rangeOfString:currentInstalledReleaseHash]
          .location == NSNotFound) {
    MSLogDebug([MSDistribute logTag],
               @"Stored release hash(es) (%@) doesn't match current "
               @"installation hash (%@), "
  ...
  MSLogDebug([MSDistribute logTag],
             @"Successfully reported app update for downloaded release hash "
             @"(%@), removing from store.",
...
getReportingParametersForUpdatedRelease:(NSString *)updateToken
  ...
  if (lastDownloadedReleaseHashes == nil) {
    MSLogDebug([MSDistribute logTag],
               @"Current release was already reported, skip reporting.");
    return nil;
  }
  ...
  if ([lastDownloadedReleaseHashes rangeOfString:currentInstalledReleaseHash]
    ...
    MSLogDebug(
        [MSDistribute logTag],
        @"New release was downloaded but not installed yet, skip reporting.");
    return nil;
  }

  // Return reporting parameters.
  MSLogDebug([MSDistribute logTag],
             @"Current release was updated but not reported yet, reporting.");
  NSMutableDictionary *reportingParameters = [[NSMutableDictionary alloc] init];
  if (updateToken) {
    reportingParameters[kMSURLQueryDistributionGroupIdKey] =
        distributionGroupId;
  } else {
    reportingParameters[kMSURLQueryInstallIdKey] =
        [[MSAppCenter installId] UUIDString];
  }
  NSString *lastDownloadedReleaseId =
      [MS_USER_DEFAULTS objectForKey:kMSDownloadedReleaseIdKey];
  reportingParameters[kMSURLQueryDownloadedReleaseIdKey] =
      lastDownloadedReleaseId;
  return reportingParameters;
}

- (void)changeDistributionGroupIdAfterAppUpdateIfNeeded:
    (NSString *)currentInstalledReleaseHash {
  NSString *updatedReleaseDistributionGroupId =
      [MS_USER_DEFAULTS objectForKey:kMSDownloadedDistributionGroupIdKey];
  if (updatedReleaseDistributionGroupId == nil) {
    return;
  }

  // Skip if the current release was not updated.
  NSString *updatedReleaseHashes =
      [MS_USER_DEFAULTS objectForKey:kMSDownloadedReleaseHashKey];
  if ((updatedReleaseHashes == nil) ||
      ([updatedReleaseHashes rangeOfString:currentInstalledReleaseHash]
           .location == NSNotFound)) {
    return;
  }

  // Skip if the group ID of an updated release is the same as the stored one.
  NSString *storedDistributionGroupId =
      [MS_USER_DEFAULTS objectForKey:kMSDistributionGroupIdKey];
  if ((storedDistributionGroupId == nil) ||
      ([updatedReleaseDistributionGroupId
           isEqualToString:storedDistributionGroupId] == NO)) {

    // Set group ID from downloaded release details if an updated release was
    // downloaded from another distribution group.
    MSLogDebug([MSDistribute logTag],
               @"Stored group ID doesn't match the group ID of the updated "
               @"release, updating group id: %@",
               updatedReleaseDistributionGroupId);
    [MS_USER_DEFAULTS setObject:updatedReleaseDistributionGroupId
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    NSString *messageFormat =
        details.mandatoryUpdate
            ? MSDistributeLocalizedString(
                  @"MSDistributeAppUpdateAvailableMandatoryUpdateMessage")
            : MSDistributeLocalizedString(
                  @"MSDistributeAppUpdateAvailableOptionalUpdateMessage");
    NSString *appName =
        [MS_APP_MAIN_BUNDLE objectForInfoDictionaryKey:@"CFBundleDisplayName"];
    if (!appName) {
      appName = [MS_APP_MAIN_BUNDLE objectForInfoDictionaryKey:@"CFBundleName"];
    }

/*
 * FIXME: Format string should be a string literal but its format is in string
 * resource so it won't be. Disable the warning temporarily.
 */
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wformat-nonliteral"
    NSString *message =
        [NSString stringWithFormat:messageFormat, appName, details.shortVersion,
                                   details.version];
#pragma clang diagnostic pop
    MSAlertController *alertController = [MSAlertController
        alertControllerWithTitle:MSDistributeLocalizedString(
                                     @"MSDistributeAppUpdateAvailable")
                         message:message];
    ...
    if (!details.mandatoryUpdate) {
      ...
      [alertController
          addDefaultActionWithTitle:MSDistributeLocalizedString(
                                        @"MSDistributeAskMeInADay")
                            handler:^(__attribute__((unused))
      ...
      [alertController
          addDefaultActionWithTitle:MSDistributeLocalizedString(
                            ...
                            handler:^(__attribute__((unused))
                                      UIAlertAction *action) {
                              MSLogDebug([MSDistribute logTag],
                                         @"'View release notes' is selected. "
                                         @"Open a browser and show release "
                                         @"notes.");

                              [MSUtility
                                   sharedAppOpenUrl:details.releaseNotesUrl
                                            options:@{}
                                  completionHandler:nil];

                              /*
                               * Clear release details so that the SDK can get
                               * the latest release again after coming back from
                               * release notes.
                               */
                              self.releaseDetails = nil;
                            }];
    ...
    [alertController
        addPreferredActionWithTitle:MSDistributeLocalizedString(
                                        @"MSDistributeUpdateNow")
                            handler:^(__attribute__((unused))
                                      UIAlertAction *action) {
     ...
     * It will replace any previous release alert. This happens when the network
     * was down so the persisted release was displayed but the network came back
...
- (void)showDistributeDisabledAlert {
  dispatch_async(dispatch_get_main_queue(), ^{
    MSAlertController *alertController = [MSAlertController
        alertControllerWithTitle:MSDistributeLocalizedString(
                                     @"MSDistributeInAppUpdatesAreDisabled")
                         message:nil];
    [alertController addCancelActionWithTitle:MSDistributeLocalizedString(
                                                  @"MSDistributeClose")
                                      handler:nil];
...
- (void)showUpdateSetupFailedAlert:(NSString *)errorMessage {
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    MSAlertController *alertController = [MSAlertController
        alertControllerWithTitle:MSDistributeLocalizedString(
                                     @"MSDistributeInAppUpdatesAreDisabled")
                         message:MSDistributeLocalizedString(
                                     @"MSDistributeInstallFailedMessage")];
    ...
    [alertController
        addDefaultActionWithTitle:MSDistributeLocalizedString(
                          ...
                          handler:^(__attribute__((unused))
                                    UIAlertAction *action) {
                            [MS_USER_DEFAULTS
                                setObject:MSPackageHash()
                                   forKey:kMSUpdateSetupFailedPackageHashKey];
                          }];

    // Add "Reinstall" button to the dialog
    [alertController
        addPreferredActionWithTitle:MSDistributeLocalizedString(
                                        @"MSDistributeReinstall")
                            handler:^(__attribute__((unused))
                                      UIAlertAction *action) {
                              NSURL *installUrl =
                                  [NSURL URLWithString:[self installUrl]];

                              /*
                               * Add a flag to the install url to indicate that
                               * the update setup failed, to show a help page
                               */
                              NSURLComponents *components = [
                                  [NSURLComponents alloc] initWithURL:installUrl
                                              resolvingAgainstBaseURL:NO];
                              NSURLQueryItem *newQueryItem = [
                                  [NSURLQueryItem alloc]
                                  initWithName:kMSURLQueryUpdateSetupFailedKey
                                         value:@"true"];
                              NSMutableArray *newQueryItems = [NSMutableArray
                                  arrayWithCapacity:[components
                                                            .queryItems count] +
                                                    1];
                              ...
                              // Open the install URL with query parameter
                              // update_setup_failed=true
                              installUrl = [components URL];
                              [self openUrlInAuthenticationSessionOrSafari:
                                        installUrl];

                              // Clear the update setup failure info from
                              // storage, to re-attempt setup on reinstall
                              [MS_USER_DEFAULTS
                                  removeObjectForKey:
                                      kMSTesterAppUpdateSetupFailedKey];
                              [MS_USER_DEFAULTS
                                  removeObjectForKey:
                                      kMSUpdateSetupFailedPackageHashKey];
                            }];
  ...
  [MSUtility sharedAppOpenUrl:details.installUrl
      ...
      completionHandler:^(MSOpenURLState state) {
        ...
        case MSOpenURLStateFailed:
          MSLogError([MSDistribute logTag],
                     @"System couldn't open the URL. Aborting update.");
          return;
        case MSOpenURLStateUnknown:

          /*
           * FIXME: We've observed a behavior in iOS 10+ that openURL and
           * openURL:options:completionHandler don't say the operation is
           * succeeded even though it successfully opens the URL. Log the result
           * of openURL and openURL:options:completionHandler and keep moving
           * forward for update.
           */
          MSLogWarning([MSDistribute logTag],
                       @"System returned NO for update but processing.");
          break;
        }

        /*
         * On iOS 8.x and >= iOS 11.0 devices the update download doesn't start
         * until the application goes in background by pressing home button.
         * Simply exit the app to start the update process. For iOS version >=
         * 9.0 and < iOS 11.0, we still need to exit the app if it is a
         * mandatory update.
         */
...
- (BOOL)openURL:(NSURL *)url {
  ...
  if (!self.appSecret ||
      ![[url scheme]
          isEqualToString:[NSString
                              stringWithFormat:kMSDefaultCustomSchemeFormat,
                                               self.appSecret]]) {
    return NO;
  }
  ...
  if ([self isEnabled]) {
    ...
    for (NSURLQueryItem *item in components.queryItems) {
      if ([item.name isEqualToString:kMSURLQueryRequestIdKey]) {
        queryRequestId = item.value;
                     ...
                     isEqualToString:kMSURLQueryDistributionGroupIdKey]) {
        ...
        queryUpdateSetupFailed = item.value;
                                ...
                                kMSURLQueryTesterAppUpdateSetupFailedKey]) {
        queryTesterAppUpdateSetupFailed = item.value;
      }
    }

    // If the request ID doesn't match, ignore.
    if (!(requestedId && queryRequestId &&
          [requestedId isEqualToString:queryRequestId])) {
      return YES;
    }
    ...
    if (queryDistributionGroupId) {
      ...
      [MS_USER_DEFAULTS setObject:queryDistributionGroupId
                           forKey:kMSDistributionGroupIdKey];

      // Update distribution group ID which is added to logs.
      [self.distributeInfoTracker
          updateDistributionGroupId:queryDistributionGroupId];

      // Only if we have managed to retrieve the Distribution group ID we should
      // update the distribution session count.
      latestSessionId =
          [[MSSessionContext sharedInstance] sessionIdAt:[NSDate date]];

      // If Analytics SDK is disabled session Id is null and there is no neeed
      // to updte the distribution session count.
      if (latestSessionId) {
     ...
     * Update token is used only for private distribution. If the query
     * parameters don't include update token, it is public distribution.
     */
    if (queryUpdateToken) {
      ...
      // Storing the update token to keychain since the update token is
      // considered as a sensitive information.
    ...
    if (queryUpdateToken || queryDistributionGroupId) {
      ...
      MSLogError([MSDistribute logTag],
                 @"Cannot find either update token or distribution group id.");
    }

    /*
     * If the in-app updates setup from the native tester app failed, retry
     * using the browser update setup.
     */
    if (queryTesterAppUpdateSetupFailed) {
      MSLogDebug([MSDistribute logTag],
                 @"In-app updates setup from tester app failure detected.");
      [MS_USER_DEFAULTS setObject:queryTesterAppUpdateSetupFailed
                           forKey:kMSTesterAppUpdateSetupFailedKey];
      [self startUpdate];
      return YES;
    }
     ...
     * the error message and also store the package hash that the failure
     * occurred on. The setup will only be re-attempted the next time the app
     * gets updated (and package hash changes).
     */
    ...
    MSLogDebug([MSDistribute logTag],
               @"Distribute service has been disabled, ignore request.");
  }
  return YES;
}
