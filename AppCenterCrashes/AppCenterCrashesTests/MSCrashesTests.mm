...
- (void)testSendOrAwaitWhenAlwaysSendIsTrue {
  ...
  OCMStub([channelGroupMock addChannelUnitWithConfiguration:[OCMArg checkWithBlock:^BOOL(MSChannelUnitConfiguration *configuration) {
      ...
      .andReturn(channelUnitMock);
  OCMStub([channelUnitMock enqueueItem:[OCMArg isKindOfClass:[MSLogWithProperties class]] flags:MSFlagsCritial])
      .andDo(^(NSInvocation *invocation) {
        (void)invocation;
        numInvocations++;
      });
...
- (void)testCrashesSetCorrectUserIdToLogs {
  ...
  OCMStub([channelGroupMock addChannelUnitWithConfiguration:[OCMArg checkWithBlock:^BOOL(MSChannelUnitConfiguration *configuration) {
      ...
      .andReturn(channelUnitMock);
  OCMStub([channelUnitMock enqueueItem:OCMOCK_ANY flags:MSFlagsCritial]).andDo(^(NSInvocation *invocation) {
    MSAbstractLog *log;
    [invocation getArgument:&log atIndex:2];
