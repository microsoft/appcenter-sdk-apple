...
- (void)testSerializationToDictionaryWorks {
  ...
  assertThat(exceptionDictionary[@"type"],
             equalTo(self.sut.modelException.type));
  assertThat(exceptionDictionary[@"message"],
             equalTo(self.sut.modelException.message));
  assertThat(exceptionDictionary[@"wrapperSdkName"],
             equalTo(self.sut.modelException.wrapperSdkName));
}

- (void)testNSCodingSerializationAndDeserializationWorks {

  // When
  NSData *serializedWrapperException =
      [NSKeyedArchiver archivedDataWithRootObject:self.sut];
  id actual =
      [NSKeyedUnarchiver unarchiveObjectWithData:serializedWrapperException];
  ...
  assertThat(actualWrapperException.exceptionData,
             equalTo(self.sut.exceptionData));

  // The exception field.
  assertThat(actualWrapperException.modelException.type,
             equalTo(self.sut.modelException.type));
  assertThat(actualWrapperException.modelException.message,
             equalTo(self.sut.modelException.message));
  assertThat(actualWrapperException.modelException.wrapperSdkName,
             equalTo(self.sut.modelException.wrapperSdkName));
}
