...
#import "MSACACAppCenterInternal.h"
#import "MSACACApplicationForwarder.h"
#import "MSACACCrashesInternal.h"
#import "MSACACCrashesPrivate.h"
#import "MSACACUtility+Application.h"

static NSString *const kMSACACAppCenterApplicationForwarderEnabledKey = @"AppCenterApplicationForwarderEnabled";

static BOOL isApplicationForwarderEnabled() {
  NSNumber *forwarderEnabled = [NSBundle.mainBundle objectForInfoDictionaryKey:kMSACACAppCenterApplicationForwarderEnabledKey];
 ...
 */
static NSString *const kMSACACCrashOnExceptionsKey = @"NSApplicationCrashOnExceptions";

static BOOL isCrashOnExceptionsEnabled() {

  // We use NSUserDefaults here instead of MSACACAppCenterUserDefaults, because
  // we should use system user defaults for system keys.
  // MSACACAppCenterUserDefaults prepends all the keys with "MSACACAppCenter" prefix.
  NSNumber *crashOnExceptions = [[NSUserDefaults standardUserDefaults] objectForKey:kMSACACCrashOnExceptionsKey];
...
typedef void (*MSACACReportExceptionImp)(id, SEL, NSException *);
static MSACACReportExceptionImp reportExceptionOriginalImp;

static void ms_reportException(id self, SEL _cmd, NSException *exception) {
  [MSACACCrashes applicationDidReportException:exception];
...
static void swizzleReportException() {
  ...
  dispatch_once(&onceToken, ^{
    ...
    reportExceptionOriginalImp = (MSACACReportExceptionImp)method_setImplementation(originalMethod, (IMP)ms_reportException);
    MSACACLogDebug([MSACACCrashes logTag], @"Selector 'reportException:' of class 'NSApplication' is swizzled.");
  });
}

#pragma mark Send Event

typedef void (*MSACACSendEventImp)(id, SEL, NSEvent *);
static MSACACSendEventImp sendEventOriginalImp;
...
static void swizzleSendEvent() {
  ...
  dispatch_once(&onceToken, ^{
    ...
    sendEventOriginalImp = (MSACACSendEventImp)method_setImplementation(originalMethod, (IMP)ms_sendEvent);
    MSACACLogDebug([MSACACCrashes logTag], @"Selector 'sendEvent:' of class 'NSApplication' is swizzled.");
  });
}

#endif

@implementation MSACACApplicationForwarder

+ (void)registerForwarding {
  if (isApplicationForwarderEnabled()) {
    MSACACLogDebug([MSACACCrashes logTag], @"Application forwarder for info.plist key '%@' enabled. It may use swizzling.",
               kMSACACAppCenterApplicationForwarderEnabledKey);
  } else {
    MSACACLogDebug([MSACACCrashes logTag], @"Application forwarder for info.plist key '%@' disabled. It won't use swizzling.",
               kMSACACAppCenterApplicationForwarderEnabledKey);
    return;
  }
#if TARGET_OS_OSX
  if (isCrashOnExceptionsEnabled()) {
    ...
    MSACACLogInfo([MSACACCrashes logTag],
