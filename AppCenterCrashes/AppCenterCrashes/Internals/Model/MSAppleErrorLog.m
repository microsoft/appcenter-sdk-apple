...
- (BOOL)isValid {
  return [super isValid] && self.primaryArchitectureId &&
         self.applicationPath && self.osExceptionType && self.osExceptionCode &&
         self.osExceptionAddress;
}

- (BOOL)isEqual:(id)object {
  if (![(NSObject *)object isKindOfClass:[MSAppleErrorLog class]] ||
      ![super isEqual:object]) {
    return NO;
  }
  MSAppleErrorLog *errorLog = (MSAppleErrorLog *)object;
  return ((!self.primaryArchitectureId && !errorLog.primaryArchitectureId) ||
          [self.primaryArchitectureId
              isEqual:errorLog.primaryArchitectureId]) &&
         ((!self.architectureVariantId && !errorLog.architectureVariantId) ||
          [self.architectureVariantId
              isEqual:errorLog.architectureVariantId]) &&
         ...
         ((!self.osExceptionAddress && !errorLog.osExceptionAddress) ||
          [self.osExceptionAddress
              isEqualToString:errorLog.osExceptionAddress]) &&
         ...
         ((!self.selectorRegisterValue && !errorLog.selectorRegisterValue) ||
          ([self.selectorRegisterValue
              isEqualToString:errorLog.selectorRegisterValue])) &&
         ((!self.threads && !errorLog.threads) ||
          [self.threads isEqualToArray:errorLog.threads]) &&
         ((!self.binaries && !errorLog.binaries) ||
          [self.binaries isEqualToArray:errorLog.binaries]) &&
         ((!self.registers && !errorLog.registers) ||
          [self.registers isEqualToDictionary:errorLog.registers]) &&
...
- (instancetype)initWithCoder:(NSCoder *)coder {
  ...
  if (self) {
    ...
    _selectorRegisterValue =
        [coder decodeObjectForKey:kMSSelectorRegisterValue];
...
- (void)encodeWithCoder:(NSCoder *)coder {
  ...
  [coder encodeObject:self.selectorRegisterValue
               forKey:kMSSelectorRegisterValue];
