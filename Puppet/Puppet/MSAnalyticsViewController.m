...
- (MSPropertyType)propertyType {
  return [(NSNumber *)objc_getAssociatedObject(self, _cmd) shortValue];
}
- (void)setPropertyType:(MSPropertyType)type {
  objc_setAssociatedObject(self, @selector(propertyType), @(type), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
- (NSString *)associatedKey {
  return objc_getAssociatedObject(self, _cmd);
}
- (void)setAssociatedKey:(NSString *)key {
  objc_setAssociatedObject(self, @selector(associatedKey), key, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
@end
...
- (IBAction)trackEvent {
  ...
  if (self.oneCollectorEnabled.on) {
    MSAnalyticsTransmissionTarget *target = [MSAnalytics transmissionTargetForToken:kMSRuntimeTargetToken];
...
- (NSUInteger)eventPropertyCount {
  return self.eventProperties.count;
}
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
  ...
  } else if ([self isEventPropertiesRowSection:section]) {
    return [self eventPropertyCount] + 1;
  } else {
...
- (NSInteger)tableView:(UITableView *)tableView indentationLevelForRowAtIndexPath:(NSIndexPath *)indexPath {
  if (([self isTargetPropertiesRowSection:indexPath.section] && ![self isTargetSelectionRowAtIndexPath:indexPath]) ||
      [self isEventPropertiesRowSection:indexPath.section]) {
