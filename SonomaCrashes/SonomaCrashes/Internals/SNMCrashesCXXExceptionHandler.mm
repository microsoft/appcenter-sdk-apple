...
typedef std::vector<SNMCrashesUncaughtCXXExceptionHandler> SNMCrashesUncaughtCXXExceptionHandlerList;
typedef struct {
  void *exception_object;
  uintptr_t call_stack[128];
  uint32_t num_frames;
} SNMCrashesCXXExceptionTSInfo;

static bool _SNMCrashesIsOurTerminateHandlerInstalled = false;
static std::terminate_handler _SNMCrashesOriginalTerminateHandler = nullptr;
static SNMCrashesUncaughtCXXExceptionHandlerList _SNMCrashesUncaughtExceptionHandlerList;
static OSSpinLock _SNMCrashesCXXExceptionHandlingLock = OS_SPINLOCK_INIT;
static pthread_key_t _SNMCrashesCXXExceptionInfoTSDKey = 0;

@implementation SNMCrashesUncaughtCXXExceptionHandlerManager
...
extern "C" void LIBCXXABI_NORETURN __cxa_throw(void *exception_object, std::type_info *tinfo, void (*dest)(void *)) {
   ...
   * initialized.
   */
  if (_SNMCrashesIsOurTerminateHandlerInstalled) {
    SNMCrashesCXXExceptionTSInfo *info =
        static_cast<SNMCrashesCXXExceptionTSInfo *>(pthread_getspecific(_SNMCrashesCXXExceptionInfoTSDKey));

    if (!info) {
      info = reinterpret_cast<SNMCrashesCXXExceptionTSInfo *>(calloc(1, sizeof(SNMCrashesCXXExceptionTSInfo)));
      pthread_setspecific(_SNMCrashesCXXExceptionInfoTSDKey, info);
    }
...
SNMCrashesIterateExceptionHandlers_unlocked(const SNMCrashesUncaughtCXXExceptionInfo &info) {
  for (const auto &handler : _SNMCrashesUncaughtExceptionHandlerList) {
    handler(&info);
  }
}

static void SNMCrashesUncaughtCXXTerminateHandler(void) {
  SNMCrashesUncaughtCXXExceptionInfo info = {
      .exception = nullptr,
  ...
  OSSpinLockLock(&_SNMCrashesCXXExceptionHandlingLock);
  {
    if (p) { // explicit operator bool
      ...
      SNMCrashesCXXExceptionTSInfo *recorded_info =
          reinterpret_cast<SNMCrashesCXXExceptionTSInfo *>(pthread_getspecific(_SNMCrashesCXXExceptionInfoTSDKey));
      ...
      try {
        ...
        SNMCrashesIterateExceptionHandlers_unlocked(info);
        ...
        OSSpinLockUnlock(&_SNMCrashesCXXExceptionHandlingLock);
...
+ (void)addCXXExceptionHandler:(SNMCrashesUncaughtCXXExceptionHandler)handler {
  ...
  dispatch_once(&key_predicate, ^{
    pthread_key_create(&_SNMCrashesCXXExceptionInfoTSDKey, free);
  });

  OSSpinLockLock(&_SNMCrashesCXXExceptionHandlingLock);
  {
    if (!_SNMCrashesIsOurTerminateHandlerInstalled) {
      _SNMCrashesOriginalTerminateHandler = std::set_terminate(SNMCrashesUncaughtCXXTerminateHandler);
      _SNMCrashesIsOurTerminateHandlerInstalled = true;
    }
    _SNMCrashesUncaughtExceptionHandlerList.push_back(handler);
  }
  OSSpinLockUnlock(&_SNMCrashesCXXExceptionHandlingLock);
}
...
+ (void)removeCXXExceptionHandler:(SNMCrashesUncaughtCXXExceptionHandler)handler {
  OSSpinLockLock(&_SNMCrashesCXXExceptionHandlingLock);
  {
    ...
    if (_SNMCrashesIsOurTerminateHandlerInstalled) {
      if (_SNMCrashesUncaughtExceptionHandlerList.empty()) {
        std::terminate_handler previous_handler = std::set_terminate(_SNMCrashesOriginalTerminateHandler);

        if (previous_handler != SNMCrashesUncaughtCXXTerminateHandler) {
          std::set_terminate(previous_handler);
        } else {
          _SNMCrashesIsOurTerminateHandlerInstalled = false;
          _SNMCrashesOriginalTerminateHandler = nullptr;
        }
      }
    }
  }
